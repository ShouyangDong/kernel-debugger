...
class AttachFullFuncArgs(ASTRewrite):
    ...
    def match(
        ...
        return isinstance(node, ast.FunctionDef) and (
            node.args.vararg is None or node.args.kwarg is None
        )
...
class ASTPat:
    ...
    @classmethod
    def from_code(
        cls, kind: ASTPatKind, code: str, placeholders: set[str]
    ) -> "ASTPat":
        if kind == "expr":
    ...
    def match_placeholders(
        self, node: "ast.AST | list[ast.AST]"
...
def attach_rewrites(
    tree: ast.AST, rewrites: list[ASTRewrite]
...
def apply_rewrites(
    tree: ast.AST, target_labels: set[int]
...
class PDD:
    ...
    @staticmethod
    def _update_probas(
        probas: dict[int, float], task: Task, is_interesting: bool
    ):
        if is_interesting:
            ...
            for label in task.applied:
                ...
                if p >= 1.0:
                    continue
                probas[label] = (
                    1.0 - (1.0 - p) / denorm if denorm > 0.0 else 0.0
                )
    ...
    def generator(self) -> Iterable[Task]:
        ...
        while True:
            choices = sorted(
                probas.items(), key=lambda x: (x[1], x[0]), reverse=True
            )
            selected = []
            ...
            task = Task(
                source=None, applied=list(applied), masked=list(masked)
            )
            if selected_count * prod == 0 or all(
                probas[label] >= 1.0 for label in applied
            ):
                break
            yield deepcopy(task)
...
def ruff_fix_code(
    code_string: str, fix_lint: bool = True, format_code: bool = True
) -> str:
    ...
    try:
        ...
        if format_code:
            ...
            subprocess.run(
                [ruff_executable, "format", tmp_path],
                capture_output=True,
                check=False,
            )
...
class Ruff(TaskManager):
    def __init__(
        self, source: str, fix_lint: bool = True, format_code: bool = True
    ):
        self.source = source
...
def _worker_loop(input_queue, output_queue):
    while True:
        try:
            ...
            output_queue.put(
                (capture_out.getvalue(), capture_err.getvalue(), success)
            )
...
class AsyncPythonRunner:
    ...
    def stop_proc(self):
        if self.process:
            ...
            # Note: if the queue is full or broken, put may block, so wrap it
            # in a try.
    ...
    async def run(self, code: str, timeout: float = 5.0):
        async with self.lock:
            ...
            while True:
                ...
                # 2. Check whether the child process is still alive (avoid
                # hanging if it segfaults).
                if not self.process.is_alive():
                    # Try one last read (in case the result was just written
                ...
                try:
                    # get_nowait raises queue.Empty immediately if the queue is
                    # empty.
                    result = self.output_queue.get_nowait()
                    return result
                except queue.Empty:
                    # No data in the queue yet, sleep briefly and yield control back to the event loop.
                    # A 0.05s delay is perfectly acceptable for interactive
                    # usage.
                    await asyncio.sleep(0.05)

    def _handle_timeout(self, timeout):
        """Handle cleanup logic when a timeout happens."""
        # We must force-terminate because exec may still be stuck in a tight
        # loop.
        if self.process and self.process.is_alive():
            self.process.terminate()
        ...
        # Mark as None so that the next run triggers start_proc and restarts
        # the worker.
        self.process = None
        self.input_queue = None
        self.output_queue = None
...
class SubProcRunner:
    ...
    async def run(self, code: str, timeout: float = 5.0):
        ...
        result = await asyncio.get_running_loop().run_in_executor(
            None, run_subprocess, ["python3", f.name]
        )
...
def clean_empty_pass(code: str) -> str:
    ...
    class PassRemover(ast.NodeTransformer):
        def clean_body(
            self, body: list[ast.stmt], keep_one=True
        ) -> list[ast.stmt]:
            if body is None:
                return None
    ...
    async def worker(self, wid: int):
        ...
        with runner:
            while True:
                ...
                if task is None:
                    break
                out, err, ok = await runner.run(
                    task.source, timeout=self.timeout
                )
...
async def main(args: Args):
    ...
    int_reduce = IntConstApply(
        lambda x: x > 1, lambda x: x // 2, "int-reduce-2"
    )
    ...
    try:
        while True:
            ...
            while await manager.run_with(ASTPDD, simplifiers):
                changed = True
            while await manager.run_with(
                ASTPDD, [int_reduce], allow_larger=True
            ):
                changed = True
...
def cli_main(argv: "Sequence[str] | None" = None) -> None:
    ...
    parser = ArgumentParser(
        usage="python -m src.auto_debug source --err-msg MSG -o OUTPUT [--backend {runner,subproc}] [--timeout SEC] [-j N]",
    ...
    parser.add_argument(
        "--backend",
        default="runner",
        choices=["runner", "subproc"],
        help="Backend for running code: runner is faster, subproc is stable",
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=60,
        help="Timeout for each task in seconds (default: 60)",
    )
    parser.add_argument(
        "-j",
        "--jobs",
